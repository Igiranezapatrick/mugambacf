...
export function RequestForm({ requestType, product, compact = false, onClose }: RequestFormProps) {
  ...
  return (
    <form onSubmit={submit} className={compact ? "grid gap-4" : "grid gap-4 border border-espresso/15 bg-white p-6 shadow-lg"}>
      ...
      <button type="submit" disabled={status === "saving"} className="inline-flex items-center justify-center gap-2.5 bg-roast px-6 py-3.5 text-sm sm:text-base font-bold uppercase tracking-widest text-white transition hover:bg-espresso disabled:cursor-not-allowed disabled:opacity-60">
