...
export function SectionIntro({ eyebrow, title, body, align = "left", titleSize = "text-3xl sm:text-7xl", eyebrowSize = "text-sm sm:text-base", bodyClassName = "" }: SectionIntroProps) {
  return (
    <div className={align === "center" ? "mx-auto max-w-4xl text-center" : "max-w-4xl"}>
      ...
      {body ? <p className={`mt-4 sm:mt-8 text-base sm:text-xl leading-relaxed text-espresso/75 border-l-4 border-brass pl-4 sm:pl-6 ${bodyClassName}`}>{body}</p> : null}
    </div>
  );
}
